...
- [토큰 등급 규칙 개정 공지](https://support.tronscan.org/hc/en-us/articles/34043051339161-Announcement-on-Revising-Token-Rating-Rules-on-TRONSCAN)
- [보안 서비스 API 출시](https://support.tronscan.org/hc/en-us/articles/20919503365273-Announcement-on-Launching-Security-Service-APIs-on-TRONSCAN)
- [노드 구축](https://support.tronscan.org/hc/en-us/articles/360027050851-How-to-Build-a-Node)
...
| **Runs** | 최적화 사용 시, 배포 시 설정한 runs 수 (No면 0) |
...
### 4-1. 기본 정보 업데이트 (로고, 웹사이트, 백서 등)
- 토큰 페이지 → **Feedback** → **Update Token Basic Info**
- **발행자 지갑**으로 연결 후 수정 (서명만, 온체인 트랜잭션 없음)
- 수정 가능: 설명, 로고, 웹사이트, 이메일, 백서, 소셜
- 수정 불가: 컨트랙트 주소, 토큰명, 심볼, 총 공급량 등 (온체인 고정)

### 4-2. 토큰 등급 업데이트 (평판)
- 토큰 페이지 → **Feedback** → **Update Token Rating**
- 또는 [문의하기 > Update Token Rating](https://tronscan.org/#/tools/contactUs?feedbackType=tokenLevel)
- TronScan 계정 필요. 검토: **영업일 기준 최대 5일**

### 4-3. 토큰 가격 정보 제출
- [문의하기 > Submit Token Price Info](https://tronscan.org/#/tools/contactUs?feedbackType=tokenPrice)
- SunSwap 유동성 $100K 이상 + 거래량 조건 또는 CoinGecko/CMC 연동 시 자동 표시

---

## 4-4. 토큰 등급 평가 규칙 (Neutral 기준)

**가이드:** [토큰 등급 규칙 개정 공지](https://support.tronscan.org/hc/en-us/articles/34043051339161) | [평가 규칙 상세](https://tronscan.org/#/tokens/rating-rule)

**Neutral 등급 요건 (2024.6 개정):**
1. 공식 웹사이트, 로고, 공식 이메일, 백서 URL, **소셜 미디어 1개 이상**
2. **컨트랙트 소스 코드 검증 완료** (미검증 시 문의로 사유 설명)
3. **프로젝트 창립자·팀 공개 프로필**이 웹사이트에 게시
4. **활성 소셜/커뮤니티 채널** 1개 이상 유지
5. **Unsafe/Scam 태그 없음**
...
**가이드:** [보안 서비스 API 출시 공지](https://support.tronscan.org/hc/en-us/articles/20919503365273) | [API 문서](https://docs.tronscan.org/security-service/security-service-api)

- 계정 권한, 멀티시그, 거래, 웹사이트, **토큰**, 계정 등 보안 정보 조회
- 개발자용 API — [문의하기](https://tronscan.org/#/tools/contactUs)로 피드백 가능

---

## 10. 노드 구축

**가이드:** [How to Build a Node?](https://support.tronscan.org/hc/en-us/articles/360027050851) → TRON 공식 문서 [Deploy A Node](https://developers.tron.network/docs/deploy-the-fullnode-or-supernode) 참조

---

## 11. Basic Guide (기본 가이드)
