...
/**
 * Link USD (LUSD) - ERC20 for Ethereum / Base / Arbitrum
 * Uniswap 호환 - transfer, approve, transferFrom 표준
 */
contract LinkUSD {
    ...
    modifier notLocked(address account) {
        require(!isLocked[account], "Account locked");
        _;
    }
    ...
    function transfer(address to, uint256 amount) public notLocked(msg.sender) returns (bool) {
        ...
        require(!isLocked[to], "Recipient locked");
